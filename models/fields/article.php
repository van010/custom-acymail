 ...
 * @version    $Id: article.php 63 2016-11-13 11:30:38Z Maik Künnemann $
 */
...
class JFormFieldModal_Article extends JFormField
{
	...
	protected function getInput() {
		...
		$html[] = '<input type="text" class="uk-button" id="' . $this->id . '_name" value="' . $title . '" disabled="disabled" size="35" />';
		$html[] = '<button href="#modalArticle' . $this->id . '" class="uk-button hasTooltip" role="button"  data-toggle="modal" title="'
			. JText::_('COM_CONTENT_CHANGE_ARTICLE') . '">'
		...
		// Edit article button
		if ($allowEdit) {
			$html[] = '<button class="uk-button hasTooltip' . ($value ? '' : ' hidden')
				. '" href="index.php?option=com_content&layout=modal&tmpl=component&task=article.edit&id=' . $value . '" target="_blank" title="'
				. JText::_('COM_CONTENT_EDIT_ARTICLE') . '" ><span class="uk-icon-edit"></span></button>';
		}

		// Clear article button
		if ($allowClear) {
			$html[] = '<button id="' . $this->id . '_clear" class="uk-button' . ($value ? '' : ' hidden') . '" onclick="return jClearArticle(\'' .
				$this->id . '\')" title="' . JText::_('JCLEAR') . '"><span class="uk-icon-remove"></span></button>';
		}
