...
if(!class_exists('tradingtechnologiesTablePortfolios_daily'))require(JPATH_TT_ADMINISTRATOR.DS.'tables'.DS.'portfolios_daily.php');

jimport('joomla.application.component.modellist');

class tradingtechnologiesModelPortfolios_daily extends JModelList {

	public function __construct($config = array()) {

		if (empty($config['filter_fields'])) {
			$config['filter_fields'] = array(
				'pd.portfolio_id', 'pd.pnl', 'pd.amount', 'pd.date',
			);
		}
	...
	public function getTable($type = 'portfolios_daily', $prefix = 'tradingtechnologiesTable', $config = array())
	{
	...
	protected function populateState($ordering = null, $direction = null) {
		...
		parent::populateState('pd.date', 'desc');
	}
	...
	protected function getStoreId($id = '') {
		...
		$id	.= ':'.$this->getState('filter.portfolio_id');
	...
	protected function getListQuery() {
		...
		$query
			...
			->from($db->quoteName('#__tt_portfolios_daily_view', 'pd'));
	...
	public function getDailyPortfolioByDate($date, $portfolio_id = 1) {
		...
		$query
			...
			->from($db->quoteName('#__tt_portfolios_daily', 'pd'));

		$query->where($db->quoteName('pd.date') .' = "'. $date .'"');
		$query->where($db->quoteName('pd.portfolio_id') .' = "'. $portfolio_id .'"');
...
/*
	...
	public function save($data) {
		...
		if (empty($data['id']) && $exist = $this->getDailyPortfolioByDate($data['date'], $data['portfolio_id'])) {
			$data['id'] = $exist->id;
		}
