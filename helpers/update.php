...
class UpdateHelper extends acymObject
{
    ...
    public function addUpdateSite($extension = null)
    {
        ...
        if (empty($extension)) {
            $object->location .= 'component?extension=acymailing&cms=joomla&version=latest&level=enterprise';
            $type = 'component';
            $folder = '';
            $element = 'com_acym';

            if (acym_level(ACYM_ESSENTIAL)) {
                $object->location .= '&website='.urlencode(ACYM_LIVE);
            }
        } else {
            ...
            $object->location .= 'extension?version=9.4.1&type='.$type.'&folder='.$folder.'&element='.$element;
        }
